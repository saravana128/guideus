...
export const taskService = {
  ...
  async updateTask(taskId, taskData, existingImageUrl = null) {
    ...
    try {
      ...
      if (existingImageUrl && data.imageUrl !== existingImageUrl && !data.imageUrl) {
  ...
  async toggleComplete(task) {
    ...
    try {
      ...
      const data = {
        completed,
        status: completed ? TASK_STATUS.COMPLETED : (task.status === TASK_STATUS.COMPLETED ? TASK_STATUS.PENDING : task.status),
        updatedAt: new Date().toISOString()
      }
