      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            <span
              className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${TASK_STATUS_COLORS[task.status] || TASK_STATUS_COLORS.pending}`}
            >
          ...
          <Link to={`/tasks/${task.$id}`}>
            <h3
              className={`text-lg font-semibold text-gray-900 hover:text-primary-600 ${task.completed ? "line-through text-gray-500" : ""}`}
            >
              {task.title}
            </h3>
          </Link>
