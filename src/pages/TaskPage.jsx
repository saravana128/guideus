      ...
      <div className="card">
        <div className="flex items-start justify-between gap-4 mb-6">
          <div>
            ...
            <h1 className={`text-2xl md:text-3xl font-bold ${task.completed ? 'line-through text-gray-500' : 'text-gray-900'}`}>
              {task.title}
            </h1>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6 text-sm">
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <p className={`font-medium ${task.status === 'overdue' ? 'text-red-600' : 'text-gray-900'}`}>
              {formatDateTime(task.dueDate)}
            </p>
          </div>
          <div className="bg-gray-50 rounded-lg p-4">
            <p className="text-gray-500 mb-1">Created</p>
            <p className="font-medium text-gray-900">{formatDateTime(task.createdAt)}</p>
          </div>
        </div>
