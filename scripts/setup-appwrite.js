...
async function setup() {
  ...
  try {
    ...
    await databases.createCollection(databaseId, tasksCollectionId, 'Tasks', [
      Permission.create(Role.users())
    ], true)
    console.log(`✅ Created collection "${tasksCollectionId}" with document security`)
  }
  ...
  for (const attr of attributes) {
    try {
      if (attr.type === 'string') {
        await databases.createStringAttribute(databaseId, tasksCollectionId, attr.key, attr.size, attr.required, attr.default || undefined, attr.key !== 'status')
      } else if (attr.type === 'datetime') {
        await databases.createDatetimeAttribute(databaseId, tasksCollectionId, attr.key, attr.required)
      } else if (attr.type === 'boolean') {
        await databases.createBooleanAttribute(databaseId, tasksCollectionId, attr.key, attr.required, attr.default ?? undefined)
      }
  ...
  for (const idx of indexes) {
    try {
      await databases.createIndex(databaseId, tasksCollectionId, idx.key, idx.type, idx.attributes)
  ...
  try {
    ...
    await storage.createBucket(storageBucketId, 'Task Attachments', [
      Permission.read(Role.any()),
      Permission.create(Role.users()),
      Permission.update(Role.users()),
      Permission.delete(Role.users())
    ], false, true, 10 * 1024 * 1024, ['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg'])
